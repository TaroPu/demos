<template>
  <div class="snowBg">
    <canvas ref="cvs" style="background-color: #000;"></canvas>
...
export default {
  ...
  methods:{
    init(){
      ...
      const awaitSentText = async (txt,fonSize,stepV) => {
        return new Promise(resolve => {
          sendText(txt,fonSize,stepV);
      ...
      const run = async () => {
        const text = `登高 - 杜甫,
          ...
          无边落木萧萧下,不尽长江滚滚来,
          万里悲秋常作客,百年多病独登台,
          艰难苦恨繁霜鬓,潦倒新停浊酒杯`.split(',');
        for (let i = 0; i < text.length; i++) {
          await awaitSentText(text[i],150,i=== 0 ? 100 : 40);
        }
      }
      run();
    }
